...
const Database = () => {
  ...
  const checkAuthStatus = async () => {
    try {
      // Check your custom authentication method to determine if the user is authenticated
      // For example, you might have a token stored in local storage or a session cookie
      ...
      if (token) {
        ...
        navigate('/App');; // Redirect to login page if not authenticated
      }
    } catch (error) {
      console.error('Error checking authentication status:', error);
      navigate('/App'); // Redirect to login page on error or if not authenticated
    }
  }

  const handleLogout = () => {
    // Your custom logout logic here, e.g., clearing local storage, session cookies, etc.
